...
import { getAllEventsAction, getTicketsAction } from "../services/actions/userActions";
  ...
  return (
    <Layout>
      ...
      <div className="container mx-auto mt-32 px-4 tessera-header relative z-10">
        <h1 className="text-3xl font-bold mb-4 tessera-header text-white">QRCODE Tickets</h1>
            ...
            >
              ...
              <div className="flex flex-col lg:flex-row justify-between align-baseline">
                <div className="flex-[0.5] text-gray-400">
                  <p className="text-[10px]">Event: {eventImage(nftTicket?.event_id)?.name}</p>
                  <p className="text-[10px]">Location: {eventImage(nftTicket?.event_id)?.location}</p>
                  <p className="text-[10px]">Date Paid: {nftTicket?.date.slice(0, 25)}</p>
                  <p className="text-[10px]">Ticket Owner: {nftTicket?.customer_name}</p>
                  <p className="text-[10px]">Payer Address: {nftTicket?.payer_address}</p>
                </div>
